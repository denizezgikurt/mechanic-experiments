...
import fontRegular from "./assets/MaccabiBlock-Regular.otf";
import fontHeavy from "./assets/MaccabiBlock-Bold.otf";
...
export const handler = ({ inputs, mechanic, sketch }) => {
  ...
  sketch.preload = () => {
    if (blob1) {
      img = sketch.loadImage(URL.createObjectURL(blob1));
    }
    if (blob2) {
      img2 = sketch.loadImage(URL.createObjectURL(blob2));
    }
  ...
  sketch.draw = () => {
    ...
    //TO-DO: Loads AIxDesign logo in corner

    //To-DO: Load X_logo, animate to minimize and then maximize later
